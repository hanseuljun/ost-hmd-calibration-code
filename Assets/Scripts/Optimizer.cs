...
public class Optimizer : MonoBehaviour {
	...
	public static void Optimize(List<Vector3> p, List<Vector3> v,
		...
		s = OptimizeScale (pp, vv);
		print ("s: " + s);

		q = OptimizeRotation (pp, vv);
	...
	public static void Optimize(List<Vector3> p, List<Vector3> v, Quaternion q,
	                            out float s, out Vector3 t) {
		if (p.Count != v.Count) {
			Debug.LogError ("sizes don't match");
			throw new ArgumentException();
		}
	...
	private static float OptimizeScale(Vector3[] pp, Vector3[] vv) {
		if (pp.Length != vv.Length) {
			Debug.LogError ("wrong lengths");
		}
		...
		return Mathf.Sqrt (sNu / sDe);
	}

	private static Quaternion OptimizeRotation(Vector3[] pp, Vector3[] vv) {
		if (pp.Length != vv.Length) {
			Debug.LogError ("wrong lengths");
		}
...
//		}
		...
		return new Quaternion (eigenVectors.Get<float> (1, 0),
