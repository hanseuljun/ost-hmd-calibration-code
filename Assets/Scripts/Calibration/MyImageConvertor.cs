...
public class MyImageConvertor
{
	...
	private Color getColor(int id, ushort depthValue)
	{
		return new Color(depthValue * floatConvertor, depthValue * floatConvertor, depthValue * floatConvertor, 1);
	}
	
	public bool generateDepthImage(IImageData image, IImageData idImage, ref Texture2D destinationImage)
	{
		...
		for (int destinationIndex = 0; destinationIndex < _colored_image.Length; ++destinationIndex)
		{
			...
			_colored_image[destinationIndex] = getColor(idImageRaw[labelIndex], value);
		}
